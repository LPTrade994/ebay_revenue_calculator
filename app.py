...
def get_final_value_fee_rate_and_group(category_id, total_sale_price):
    ...
    if not fvf_group_data:
        st.warning(f"ID Categoria {category_id} non trovato, si applica 'Altre categorie'.")
    ...
    if 'variable_rate' in fvf_group_data:
        ...
        price_processed_up_to = Decimal('0')
...
def calculate_fees(item_price, shipping_cost, item_cost, category_id, buyer_country,
                   ...
                   num_listings_this_month, listing_type, 
    ...
    country_to_fee_key_map = {
        "Italia": "Eurozone_Sweden", "Malta": "Eurozone_Sweden", "Germania": "Eurozone_Sweden",
        "Francia": "Eurozone_Sweden", "Spagna": "Eurozone_Sweden", "Svezia": "Eurozone_Sweden",
        "Regno Unito": "United_Kingdom", "Stati Uniti": "United_States_Canada", "Canada": "United_States_Canada",
        "Svizzera": "Europe_non_eurozone_Sweden_UK", "Norvegia": "Europe_non_eurozone_Sweden_UK",
        "Altro (Resto del Mondo)": "Rest_of_world"
    }
    international_fee_rate_key = country_to_fee_key_map.get(buyer_country, "Rest_of_world")
    ...
    if not is_vehicle_insertion:
        ...
        if store_subscription == "Nessuno":
            ...
            free_allowance = store_data.get(free_listings_key)
            if free_allowance == "unlimited":
                insertion_fee = Decimal('0')
                insertion_fee_details = f"Inserzioni '{listing_type}' illimitate ({store_subscription})"
                ...
                insertion_fee_details = f"Extra '{listing_type}' ({store_subscription}, >{free_allowance} gratuiti)"
            elif isinstance(free_allowance, int):
                insertion_fee = Decimal('0')
                insertion_fee_details = f"Gratuita '{listing_type}' ({store_subscription}, quota {free_allowance})"
            else: 
                insertion_fee = to_decimal(store_data.get(extra_listing_fee_key, '0'))
                insertion_fee_details = f"Tariffa '{listing_type}' ({store_subscription}, verifica quota)"
    ...
    if use_reserve_price and reserve_price_value > 0 and listing_type == "Asta":
        ...
        if is_vehicle_cat_for_reserve and "vehicle_reserve_price_fee" in FEE_DATA['vehicles']:
            ...
            reserve_fee_detail = f"Tariffa fissa prezzo di riserva veicoli: {reserve_fee} €"
        else: 
            rp_config = FEE_DATA['listing_upgrades']['reserve_price']
            reserve_price_dec = to_decimal(reserve_price_value)
            reserve_fee = max(to_decimal(rp_config['min_fee']), 
                              min(to_decimal(rp_config['max_fee']), 
                                  reserve_price_dec * to_percentage_decimal(rp_config['percentage_rate'])))
            reserve_fee_detail = (f"{rp_config['percentage_rate']*100}% su {reserve_price_dec}€, "
...
with col1:
    ...
    if category_id_input == 0:
        category_id_input = st.number_input("ID Categoria eBay (se 'Altro')", min_value=1, value=example_category_id)
    else:
        st.caption(f"ID Categoria: {category_id_input}")

with col2:
    buyer_country_options = [
        "Italia", "Malta", "Germania", "Francia", "Spagna", "Svezia", 
        "Regno Unito", "Stati Uniti", "Canada", "Svizzera", "Norvegia",
        "Altro (Resto del Mondo)"
    ]
...
with col3:
    ...
    if store_subscription_input != "Nessuno":
        ...
        if isinstance(free_allowance_help, int): default_listings_val = free_allowance_help + 1
    num_listings_input = st.number_input(f"N° inserz. '{listing_type_input}' mese?", min_value=1, value=default_listings_val, step=1, help="Rilevante per quote gratuite negozio.")

with col4:
    st.write("**Opzioni Vendita:**")
...
if st.sidebar.button("🔄 Calcola", use_container_width=True):
    ...
    with res_col2:
        ...
        if current_is_vehicle_fixed_fvf:
            fvf_rate_display = "Fissa Veicolo"
        elif current_fvf_group_data:
            if 'variable_rate' in current_fvf_group_data:
                 fvf_rate_display = f"{current_fvf_group_data['variable_rate']*100:.1f}%"
            elif 'tiers' in current_fvf_group_data:
                fvf_rate_display = "A Scaglioni"

        st.text(f"CVF Base ({fees['fvf_group_name']} - {fvf_rate_display}):")
        st.text(f"{fees['total_sale_price']:.2f} € -> {example_fvf_base:.2f} €")
        ...
        st.text(f"Commissione valore finale (netta): {net_fvf_for_example:.2f} €")
        st.markdown("---")
        reg_fee_rate_perc = FEE_DATA['constants']['regulatory_compliance_fee_rate'] * 100
        st.text(f"Adeguamento normativo ({reg_fee_rate_perc:.2f}%): {fees['regulatory_fee']:.2f} €")
        st.markdown("---")
        ...
        st.markdown(f"**Tariffe totali per oggetto: {total_fees_per_item_example:.2f} €**")
        st.markdown("---")
        ...
        if apply_vat_input:
            vat_on_example_style = to_decimal(total_fees_pre_vat_example_style * (Decimal(str(vat_rate_val_input)) / 100))
            st.text(f"IVA ({vat_rate_val_input:.1f}%): {vat_on_example_style:.2f} €")
            st.markdown(f"**Tariffe totali (IVA inclusa): {total_fees_pre_vat_example_style + vat_on_example_style:.2f} €**")
